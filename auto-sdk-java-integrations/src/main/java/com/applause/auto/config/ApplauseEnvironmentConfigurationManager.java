...
/** thread-safe utility class for loading the Applause configuration. */
public enum ApplauseEnvironmentConfigurationManager {
  /** The singleton instance of the ApplauseConfigurationManager */
  INSTANCE;

  private Supplier<ApplauseSdkConfigBean> configCache = createConfigBean();
   ...
   */
  public ApplauseSdkConfigBean get() {
  ...
  private Supplier<ApplauseSdkConfigBean> createConfigBean() {
    return Suppliers.memoize(
        () ->
            // order is critical here. order is in 'most->least' priority
            ConfigFactory.create(
                ApplauseSdkConfigBean.class,
                Optional.ofNullable(overwrittenProps).orElse(new HashMap<>()),
